  ...
  <v-app light>
    ...
    <nav>
      <v-toolbar id="navbar">
          ...
          <v-toolbar-title>
            <span class="font-weight-light pr-3">Excel Viewer </span>
          </v-toolbar-title>
          <v-spacer></v-spacer>
         <!-- <v-icon to="/" @click="logout()">mdi-logout</v-icon> -->
      </v-toolbar>
    </nav>
    <v-navigation-drawer app v-model="side" fixed v-if="$vuetify.breakpoint.smAndUp">
      <template v-slot:prepend>
        <v-list-item two-line>
          ...
          <v-list-item-content>
            <v-list-item-title>JAMES A</v-list-item-title>
          </v-list-item-content>
    ...
    <v-btn value="favorites" :to="'/policies'">
      <span>Policies</span>
      <v-icon>mdi-heart</v-icon>
    </v-btn>
    <v-btn value="nearby" :to="'/load_data'">
      <span>Upload</span>
  ...
  data () {
    return {
      ...
      items: [
        ...
        { text: 'Policies', icon: 'mdi-cart', link: '/policies' },
        { text: 'Load Data', icon: 'mdi-file', link: '/load_data' }
      ]
    }
  },
  methods: {
